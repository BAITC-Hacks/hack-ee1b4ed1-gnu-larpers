...
function ClientDetails({ node, data }: { node: NodeRow; data: GraphData }) {
  ...
  return <aside className="details-panel" aria-label="Карточка клиента">
    ...
    <section className="evidence-section"><h3><ShieldCheck size={15} /> Почему эта роль</h3><p>{readableEvidence(node)}</p><div className="support-line"><span>Поддержка гипотезы</span><strong>{node.role_score.toFixed(3)} / 1</strong></div><details className="raw-evidence"><summary>Показать исходные признаки</summary><p>{node.evidence}</p></details></section>
...
export default function InvestigationPage({ data, node, onSelectClient, onBrowseClients, onOpenMethod }: { data: GraphData; node: NodeRow; onSelectClient: (gid: string) => void; onBrowseClients: () => void; onOpenMethod: () => void }) {
  return <>
    ...
    <Transactions data={data} node={node} onSelect={onSelectClient} />
  </>;
}
