...
Python-бэкенд и React-интерфейс для анализа внутрибанковских переводов. Читает три Parquet-файла, проверяет согласованность, определяет наблюдаемые роли клиентов, выделяет группы и формирует CSV для сдачи и JSON для интерфейса. Базовый анализ работает локально без LLM. Дополнительный режим AI-агента использует локальный API и внешний LLM API.
